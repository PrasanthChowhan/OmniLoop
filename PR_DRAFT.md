...
**Proposed Changes:**
*   `src/Workspace.ts`: Added `progressFile` to manage the durable `omniloop-progress.txt` state artifact.
*   `src/ContextSynthesizer.ts`: Integrated the injection of `omniloop-progress.txt` into the agent context string to enable accurate shift handoffs.
*   `src/SprintOrchestrator.ts`:
    *   Replaced blind `substring(0, max)` truncations with suffix-preserving truncations (e.g., `substring(feedback.length - max)`) for `sprint_feedback.md` and `sprint_contract.md`.
    *   Separated `generatorContext` and `evaluatorContext` so the Evaluator runs in a pristine environment without inheriting the Generator's prior feedback baggage.
*   `src/index.ts`:
    *   Added a loop breaker (`if (!success) break;`) to gracefully halt execution and prevent infinite loops when a sprint fails `maxCycles`.
    *   Removed `spawnSync` auto-execution for `.omniloop/init.sh`, replacing it with a security warning requiring explicit user validation.
