...
1.  **Review the Blueprint**: Read `.omniloop/blueprint.json` (`visual_identity` is your grading rubric) and `.omniloop/sprint_contract.md`.
2.  **Empirical Testing**: 
...
5.  **The Verdict**:
    - **PASS**: If all criteria are met, update `.omniloop/blueprint.json` (`"passes": true`) and clear `.omniloop/sprint_feedback.md`.
    - **FAIL**: If any criteria fail, keep the feature as `"passes": false`. This will trigger a new **Cycle** for the current **Sprint**. Write a detailed, "gritty" critique in `.omniloop/sprint_feedback.md`. Include terminal output from failed tests and specific UI flaws.
